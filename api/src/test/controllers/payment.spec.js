  ...
  describe('#Payment makepayment()', function () {
    it('Create Mangopaylegal  user  for Formation Center', function (done) {
      ...
      FormationCenter.find().exec(function (err, formationCenterArray) {
        ...
        iFormationCenter = formationCenter = formationCenterArray[0]
    ...
    //       dateToUser =  new Date() // moment('300681', 'DDMMYY').unix()
    //
    //       CustomerToInsert = {
    ...
    //       //moment(currentDate).format('MMYY')
    ...
    //         CardExpirationDate:currentDate  ,  //12-06-2023
    //         CardCvx: '342',
    //       }
    //       var testCurrency = "EUR"
    //       var mount = 1000
    ...
    it('Make payment from user to Formation Center wallet with Formation information', function (done) {
      ...
      FormationCenter.findOne({name:  formationcentername}).populate("formations").exec(function (err, formationCenter) {
        userid = faker.finance.account()
        walletid = faker.finance.account()
        console.log("Call services for Formation")
        Formation.find({}).exec(function (err, updateObjectArray){
          updateObject = formationCenter.formations[0]
          console.log("Call services for Formation inner", updateObject)
          var nameFormationCenter = updateObject.id
          //birtDate = faker.date.past();
          dateToUser =  new Date()


          console.log("Call services for Formation point")
         ...
         // moment(currentDate).format('MMYY')
          ...
          cardValueEx = {
            CardNumber: '4706750000000017',
            CardExpirationDate:currentDate   ,  //12-06-2023  moment('0623', 'MMDD').unix() currentDate
            CardCvx: '342',
          }
          var testCurrency = "EUR"
          var mount = 1000
          ...
          //Validate price
console.log("Call services for Formation these")
